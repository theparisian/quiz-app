...
export function resetPrizeRedeemRateLimitForTests(): void {
...
export function checkPrizeRedeemRateLimit(clientKey: string): {
  allowed: boolean;
  resetAt: Date | null;
} {
  const now = Date.now();
...
export function clientKeyFromIp(ip: string | undefined): string {
