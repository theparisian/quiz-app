...
export default function SponsorPrizesConfigPage() {
  const params = useParams();
  const slug = typeof params.slug === 'string' ? params.slug : '';
  const qc = useQueryClient();

  const q = useQuery({
    queryKey: ['sponsor-prizes-config', slug],
    queryFn: () => api.get<ConfigResponse>(`/api/sponsors/${slug}/prizes-config`),
    enabled: !!slug,
  });
  ...
  const mut = useMutation({
    mutationFn: (body: { config: Record<string, unknown> }) =>
      api.patch(`/api/sponsors/${slug}/prizes-config`, body),
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['sponsor-prizes-config', slug] }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Lots du sponsor</h1>
      </div>
      <p className="text-sm text-gray-600">
        Ces lots s&apos;appliquent aux sessions dont le quiz est lié à ce sponsor (priorité sur les
        lots du cinéma).
      </p>
