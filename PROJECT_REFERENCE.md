...
- **Identification :** chaque NUC a un `nuc_uid` unique généré côté serveur lors de la création du device dans le super‑admin (`POST /screens/:screenId/nucs`). En cabine ce même identifiant est reflété localement sous `/etc/quiz-app/nuc-id`.
- **Provisionnement navigateur & cookie:** la page `apps/player` appelle après scan initial `POST /api/nucs/auth` ; le serveur pose un JWT HTTPOnly **`nuc_session`**.
- **Heartbeat :** après provisionnement navigateur : **`POST /api/nucs/heartbeat`** (cookie `nuc_session`, corps `{}` depuis le bundle player). Une route **`POST /api/nuc/heartbeat`** (singulier) accepte encore `nucUid` + `authKey` pour intégrations hors navigateur.
...
- **Hébergement :** VPS en Union européenne (cf. §4.3 : généralement **OVH** ou équivalent UE) ; URL / identifiant d’instance de prod : **[à compléter]**
