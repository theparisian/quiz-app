...
- **Identification :** chaque NUC a un `nuc_uid` unique généré à la première mise en service, stocké dans `/etc/quiz-app/nuc-id`.
- **Heartbeat :** toutes les 30 secondes, POST `/api/nuc/heartbeat` avec ID, version, timestamp, IP.
